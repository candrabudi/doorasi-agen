...
class DistributorSearch {
  ...
  async handleSearch(keyword) {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/search-regions?keyword=${encodeURIComponent(keyword)}`
      );
  ...
  async handleOptionClick(regionId, label) {
    ...
    try {
      const response = await fetch(
        `${API_BASE_URL}/get-distributors?region=${encodeURIComponent(label)}`
      );
      const data = await response.json();
  ...
  renderDistributors(distributors) {
    ...
    if (!distributors.length) {
      this.elements.searchResultContainer.innerHTML =
        "<p>Tidak ada distributor di wilayah ini.</p>";
      return;
    }

    distributors.forEach((distributor) => {
      this.elements.searchResultContainer.appendChild(
        this.createDistributorCard(distributor)
      );
    });
  }
  ...
  createCardBody(distributor) {
    ...
    googleMaps.innerHTML = `
            ...
            <a href="${
              distributor.maps_url || distributor.google_maps_url || "#"
            }" target="_blank">Peta Google</a>
        `;

    const phone = document.createElement("p");
    phone.innerHTML = `
            <img decoding="async" loading="lazy" class="mr-2" src="${ASSET_BASE_URL}/phone.svg" alt="phone-icon">
            <a href="tel:${distributor.phone || distributor.primary_phone}">${
    ...
    cod.innerHTML = `
            ...
            ${
              distributor.cod || distributor.is_cod
